...
import { ClaimTunnelResponse, CreateTunnelRequest } from "src/proto/api";
import {
...
export const claimTunnel: RPCHandler<"claimTunnel"> = async (
  ...
  try {
    ...
    res.request!.signatures = {
      requestSignature: await signAsAttestor(
        CreateTunnelRequest.encode(claimRequest.request!).finish(),
