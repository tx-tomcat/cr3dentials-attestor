...
async function _createClaimOnAttestor<N extends ProviderName>({
  ...
  function addServerSideReveals() {
    ...
    let serverPacketsToReveal: ReturnType<
      typeof getBlocksToReveal<ServerAppDataPacket>
    > = "all";
    ...
    if (provider.getResponseRedactions) {
      serverPacketsToReveal = getBlocksToReveal(serverBlocks, (total) =>
        provider.getResponseRedactions!(total, params)
      );
    }
