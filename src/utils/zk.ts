...
export async function makeZkProofGenerator({
  ...
  async function generateProofForChunk(
    ...
    const apiUrl =
      getEnvVariable("PUBLIC_API_URL") || "https://zk-attestor-sdk.netalabs.me";

    const response = await axios.post(`${apiUrl}/api/proofs/generate`, {
      data: {
        algorithm,
        privateInput: {
          key: Buffer.from(privateInput.key).toString("base64"),
        },
        publicInput: {
          ciphertext: Buffer.from(publicInput.ciphertext).toString("base64"),
          iv: Buffer.from(publicInput.iv).toString("base64"),
    ...
    return {
      proofJson: JSON.stringify(proof.proofJson),
      decryptedRedactedCiphertext: new Uint8Array(
        Buffer.from(proof.plaintext, "base64")
      ),
      redactedPlaintext,
      startIdx,
    };
...
export async function verifyZkPacket({
  ...
  await Promise.all(
    proofs.map(
      async (
        { proofJson, decryptedRedactedCiphertext, redactedPlaintext, startIdx },
        i
      ) => {
  ...
  if (!zkOperators[algorithm]) {
    ...
    const fetcher =
      opType === "local"
        ? makeLocalFileFetch()
        : makeRemoteFileFetch({
            baseUrl: DEFAULT_BASE_REMOTE_ZK_URL,
          });
